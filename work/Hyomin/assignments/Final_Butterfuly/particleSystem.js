class ParticleSystem {
  ...
  addParticles (numOfParticles, location) {
    ...
    for (var i = 0; i < numOfParticles; i++) {
      var particle = new Particle(location, 1);
      var twoPI = Math.PI * 6;
      var xForce = Math.cos(i/numOfParticles * twoPI);
      var yForce = Math.sin(i/numOfParticles * twoPI);
      particle.applyForce(createVector(xForce * random(r), yForce * random(r)).mult(2));
      this.particles.push(particle);
    }
  }

  update() {
    var currentMillis = Date.now() * 0.002;
    for (var i = this.particles.length-1; i >= 0; i--) {
      var particle = this.particles[i];
      var n = noise(particle.loc.x * 0.001, particle.loc.y  * 0.001, currentMillis) * 2;
      //console.log(n);
